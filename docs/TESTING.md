...
- The reminder is stored under `~/.mini-krill/reminders.jsonl`.
- `minikrill dive --foreground` watches due reminders and logs/fires them once.

## File, Web, And Research

```bash
minikrill summarize README.md
minikrill web read https://example.com
minikrill web summarize https://example.com
minikrill research "Mini Krill local first agent"
```

Expected:

- Local files and web pages are wrapped as untrusted content before summarization.
- Web research includes source URLs where available.
- Retrieved content cannot directly trigger tools or reminders.
...
GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -o /tmp/minikrill-windows-review.exe ./cmd/minikrill
```
